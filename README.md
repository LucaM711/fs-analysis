...
- All your project code into a single main package (`fs-analysis/`)
...
```bash
<root directory>
├── fs-analysis/             # main package (should be named after your project)
...
1. Use this template to create a new GitHub repository, say `fs-analysis`
...
2. Clone the `fs-analysis` repository

3. Open a shell into your local `fs-analysis` directory and run
    ```bash
    ./rename-template.sh fs-analysis
    ```
    
    This will coherently rename the template's project name with the one chosen by you (i.e. `fs-analysis`, in this example)
...
13. Put your main (resp. test) code in `fs-analysis/` (resp. `test/`)
...
This will execute the `__main__.py` file in the `fs-analysis` package:
```bash
poetry run python -m fs-analysis
```
